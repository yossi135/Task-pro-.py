...
def create_ticket(username,title,description,position):
    query='''
    INSERT INTO Tickets(UserName,Title, Description,Position)
    VALUES (?,?,?,?)
    '''
...
def em_to(to_email):
    ...
    if to_email=='yossifhendy32@gmail.com':
         new='Youssef'
    elif to_email=='Kareem.Abdelhaleem@almada-eg.com':
        new='Kareem'
...
def update_assignto(ticket_id,new):
    query='''UPDATE Tickets SET assignto =? WHERE FormattedID= ? '''
...
def main_application():
    ...
    if role=='assigner':
        ...
        if menu=='Update Ticket Status':
            ...
            st.write(f'Hi {name} you need to assign the tasks based on the issue. Here are three person :-->[Youssef, Kareem, Adham] ')
            #def send_email(to_email,subject, message):
            to_email=st.selectbox('send_to',['yossifhendy32@gmail.com','Kareem.Abdelhaleem@almada-eg.com','Adham.omar@almada-eg.com'])
